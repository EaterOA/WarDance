...
Orange::Orange(sf::Vector2f pos, Item::Type drop)
...
Orange::Orange(util::ShapeVector size, sf::Vector2f pos, int hp, int faction, int bounty, Item::Type drop)
    : Enemy(size, pos, hp, faction, bounty, drop)
{
}

void Orange::cooldown(BattleState& state)
{
    Fighter::cooldown(state);
    m_turnCD -= state.elapsed.asSeconds();
}

void Orange::attack(BattleState& state)
{
    m_attackCD = RAND(500, 2000) / 1000.f;
    shoot(state, REGULAR, 30.f, 7.f);
}

void Orange::hit(BattleState &state, int damage)
{
...
void Orange::act(BattleState& state)
{
    cooldown(state);
